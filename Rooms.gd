...
preload("res://Rooms/Room2.tscn"), preload("res://Rooms/Room3.tscn"), preload("res://Rooms/Room4.tscn"),
preload("res://Rooms/Room5.tscn"), preload("res://Rooms/Room6.tscn"), preload("res://Rooms/Room7.tscn"),
preload("res://Rooms/Room8.tscn"), preload("res://Rooms/Room9.tscn")]
const SPECIAL_ROOMS: Array = [preload("res://Rooms/SpecialRoom0.tscn"), preload("res://Rooms/SpecialRoom1.tscn")]
const END_ROOMS: Array = [preload("res://Rooms/EndRoom0.tscn")] # Only one for now
const BOSS_SCENE: PackedScene = preload("res://Rooms/BossRoom.tscn")
...
func _spawn_rooms() -> void:
	...
	for i in num_levels:
		...
		if i == 0:
			...
			# Random room between the 2
			
			player.position = room.get_node("PlayerSpawnPos").position
		else:
			if i == num_levels - 1:
				room = END_ROOMS[randi() % END_ROOMS.size()].instance()
				# In this case, there is only one for now
			
			else:
				if SavedData.num_floor == 3:
				# Retrieves number of runs of player
					room = BOSS_SCENE.instance()
				else:
